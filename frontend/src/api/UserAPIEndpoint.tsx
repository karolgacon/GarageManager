...
export const UserService = {
    ...
    getClients: async (): Promise<User[]> => {
        try {
            ...
            return response.data.filter((user: User) => user.role === 'client');
        } catch (error) {
    ...
    getMechanics: async (): Promise<User[]> => {
        try {
            ...
            return response.data.filter((user: User) => user.role === 'mechanic');
        } catch (error) {
    ...
    getOwners: async (): Promise<User[]> => {
        try {
            ...
            return response.data.filter((user: User) => user.role === 'owner');
        } catch (error) {
    ...
    getUsersByRole: async (role: string): Promise<User[]> => {
        try {
            ...
            console.error(`Error fetching and filtering users by role ${role}:`, error);
            throw error;
        }
    },
};