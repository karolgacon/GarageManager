...
def test_create_payment(api_client, test_user, test_invoice):
    ...
    api_client.force_authenticate(user=test_user)
    payment_data = {
        'invoice': test_invoice.id,  # This is correct for the API request
        'amount_paid': '75.00',
        'payment_method': 'card',
        'transaction_id': str(uuid.uuid4())[:100],
    ...
    # Need to make sure serializer can handle invoice ID properly
    ...
    assert payment.invoice.id == test_invoice.id  # Verify relation is correct
...
def test_retrieve_payment(api_client, test_user, test_payment):
    ...
    # Check some key fields rather than comparing entire serialized objects
...
def test_update_payment(api_client, test_user, test_payment):
    ...
    updated_data = {
        'invoice': test_payment.invoice.id,  # This is correct for the API request
        'amount_paid': '60.00',
