...
class IsAdmin(BasePermission):
    def has_permission(self, request, view):
        return request.user.role == 'admin' and request.user.verification == 'verified'


class IsOwner(BasePermission):
    def has_permission(self, request, view):
        return request.user.role == 'owner' and request.user.verification == 'verified'


class IsVerified(BasePermission):
    def has_permission(self, request, view):
        return request.user.verification == 'verified' and (
                    request.user.role == "owner" or request.user.role == "member")
...
class JWTAuthentication(authentication.BaseAuthentication):
    ...
    def create_jwt(cls, user):
        payload = {
            ...
            'institution': user.institution.krs if user.role != "admin" and user.institution else None,
